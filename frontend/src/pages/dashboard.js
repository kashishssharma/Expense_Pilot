...
import { DollarSign, TrendingUp, TrendingDown, ShoppingBag, ArrowUpRight, ArrowDownRight } from 'lucide-react';
...
export default function Dashboard() {
  ...
  return (
    <Layout>
      <Head><title>Dashboard — ExpensePilot</title></Head>
      <h1 className="text-3xl font-bold text-white mb-8">Dashboard</h1>

      {/* ─── Stat Cards ─────────────────────────────── */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        <StatCard icon={DollarSign} label="This Month" value={`$${(o.currentMonth?.total || 0).toLocaleString()}`} sub={`${Math.abs(change)}% ${change >= 0 ? 'more' : 'less'} than last month`} trend={change >= 0 ? 'up' : 'down'} color="brand" />
